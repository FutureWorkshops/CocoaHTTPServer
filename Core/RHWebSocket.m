...
@interface RHWebSocket (PrivateAPI)
...
+ (BOOL)isWebSocketRequest:(RHHTTPMessage *)request
{
	// Request (Draft 75):
	// 
	// GET /demo HTTP/1.1
	...
	else if (![upgradeHeaderValue caseInsensitiveCompare:@"RHWebSocket"] == NSOrderedSame) {
		isWebSocket = NO;
	}
...
+ (BOOL)isVersion76Request:(RHHTTPMessage *)request
{
...
+ (BOOL)isRFC6455Request:(RHHTTPMessage *)request
{
...
- (id)initWithRequest:(RHHTTPMessage *)aRequest socket:(GCDAsyncSocket *)socket
{
	HTTPLogTrace();
...
- (void)sendResponseHeaders
{
	...
	RHHTTPMessage *wsResponse = [[RHHTTPMessage alloc] initResponseWithStatusCode:101
	                                                              description:@"Web Socket Protocol Handshake"
	                                                                  version:HTTPVersion1_1];
	
	[wsResponse setHeaderField:@"Upgrade" value:@"RHWebSocket"];
