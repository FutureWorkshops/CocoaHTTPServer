...
@interface RHHTTPServer (PrivateAPI)
...
- (id)init
{
	if ((self = [super init]))
	{
		...
		connectionClass = [RHHTTPConnection self];
...
- (void)stop:(BOOL)keepExistingConnections
{
	...
	dispatch_sync(serverQueue, ^{ @autoreleasepool {
		...
		if (!keepExistingConnections)
		{
			...
			for (RHHTTPConnection *connection in connections)
			{
				[connection stop];
			}
			...
			[webSocketsLock lock];
			for (RHWebSocket *webSocket in webSockets)
			{
				[webSocket stop];
			}
...
- (void)addWebSocket:(RHWebSocket *)ws
{
	[webSocketsLock lock];
...
- (RHHTTPConfig *)config
{
	...
	return [[RHHTTPConfig alloc] initWithServer:self documentRoot:documentRoot queue:connectionQueue];
}

- (void)socket:(GCDAsyncSocket *)sock didAcceptNewSocket:(GCDAsyncSocket *)newSocket
{
	RHHTTPConnection *newConnection = (RHHTTPConnection *)[[connectionClass alloc] initWithAsyncSocket:newSocket
