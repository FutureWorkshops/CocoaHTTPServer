...
#import "RHMultipartMessageHeaderField.h"
...
@implementation RHMultipartMessageHeader
...
- (id) initWithData:(NSData *)data formEncoding:(NSStringEncoding) formEncoding {
	...
	while( offset < length - 2 ) {
		...
		if( (*(uint16_t*) (bytes+offset)  == fields_separator) && ((offset == length - 2) || !(isspace(bytes[offset+2])) )) {
			...
			RHMultipartMessageHeaderField* field = [[RHMultipartMessageHeaderField alloc] initWithData: fieldData  contentEncoding:formEncoding];
			if( field ) {
