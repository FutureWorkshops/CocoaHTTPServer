...
#import "RHMultipartFormDataParser.h"
#import "DDData.h"
...
@interface RHMultipartFormDataParser (private)
...
@implementation RHMultipartFormDataParser 
	...
	while( true ) {
		...
		if( nil == currentHeader ) {
			...
			if( -1 == headerEnd ) {
				...
				currentHeader = [[RHMultipartMessageHeader alloc] initWithData:headerData formEncoding:formEncoding];
		...
		if( contentEnd == -1 ) {
			...
			NSData* decodedData = [RHMultipartFormDataParser decodedDataFromData:[NSData dataWithBytesNoCopy:(char*)workingData.bytes + offset length:workingData.length - offset - sizeToLeavePending freeWhenDone:NO] encoding:currentEncoding];
