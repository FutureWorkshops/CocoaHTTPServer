#import "RHDAVConnection.h"
#import "RHHTTPMessage.h"
#import "RHHTTPFileResponse.h"
#import "RHHTTPAsyncFileResponse.h"
#import "RHPUTResponse.h"
#import "RHDELETEResponse.h"
#import "RHDAVResponse.h"
...
@implementation RHDAVConnection
...
- (NSObject<RHHTTPResponse>*) httpResponseForMethod:(NSString*)method URI:(NSString*)path {
  if ([method isEqualToString:@"HEAD"] || [method isEqualToString:@"GET"]) {
    ...
    if (filePath) {
      ...
      if (fileAttributes) {
        if ([[fileAttributes objectForKey:NSFileSize] unsignedLongLongValue] > HTTP_ASYNC_FILE_RESPONSE_THRESHOLD) {
          return [[RHHTTPAsyncFileResponse alloc] initWithFilePath:filePath forConnection:self];
        } else {
    ...
    if (filePath) {
      if ([requestContentBody isKindOfClass:[NSString class]]) {
        return [[RHPUTResponse alloc] initWithFilePath:filePath headers:[request allHeaderFields] bodyFile:requestContentBody];
      } else if ([requestContentBody isKindOfClass:[NSData class]]) {
        return [[RHPUTResponse alloc] initWithFilePath:filePath headers:[request allHeaderFields] bodyData:requestContentBody];
      } else {
    ...
    NSString* filePath = [self filePathForURI:path allowDirectory:YES];
    if (filePath) {
      return [[RHDELETEResponse alloc] initWithFilePath:filePath];
    }
  }
  ...
  if ([method isEqualToString:@"OPTIONS"] || [method isEqualToString:@"PROPFIND"] || [method isEqualToString:@"MKCOL"] ||
    ...
    if (filePath) {
      ...
      return [[RHDAVResponse alloc] initWithMethod:method
